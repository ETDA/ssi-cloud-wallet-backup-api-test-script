...
describe('Get all VC in wallet by ID', () => {
  ...
  test('Get all VC in wallet by ID', async () => {
    try {
      ...
      state.vcId = getVCfromCloud.data.items[0].id

      const getVCById = await Wallet.GetVCByID(state.cid, state.data.did, state.didKey1)
      console.log('Get VC by ID: ', JSON.stringify(getVCById.data, null, 2))
      expect(getVCById.status).toEqual(200)
      expect(getVCById.data.is_exists).toBe(true)
    } catch (err) {
  ...
  test('Get all VC in wallet by ID by invalid signature', async () => {
    try {
      const issRegister = await DID.Register(KEY_TYPE.EcdsaSecp256r1VerificationKey2019)
      console.log('Issuer Register: ', JSON.stringify(issRegister.request.data, null, 2))
      expect(issRegister.request.status).toEqual(201)
      ...
      const getVCById = await Wallet.GetVCByID(state.cid, state.data.did, state.didKey2)
      console.log('Get VC by ID: ', JSON.stringify(getVCById.data, null, 2))
  ...
  test('Get all VC in wallet by ID by incorrect vc_id', async () => {
    try {
      const didRegister = await DID.Register(KEY_TYPE.EcdsaSecp256r1VerificationKey2019)
      console.log('DID Register: ', JSON.stringify(didRegister.request.data, null, 2))
      expect(didRegister.request.status).toEqual(201)
      ...
      const dummyCid = `did:idin:${uuidv4()}`
      const getVCById = await Wallet.GetVCByID(dummyCid, state.data.did, state.didKey1)
      console.log('Get VC by ID: ', JSON.stringify(getVCById.data, null, 2))
      expect(getVCById.status).toEqual(200)
      expect(getVCById.data.is_exists).toBe(false)
    } catch (err) {
      console.log(err.response)
      expect(err).not.toBeTruthy()
    }
