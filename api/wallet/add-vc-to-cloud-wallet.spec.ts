...
import { ERR_REQUIRE, INVALID, KEY_TYPE, RESULT, VC_STATUS } from '../consts'
...
describe('Add VC to cloud wallet', () => {
  ...
  test('Add VC to cloud wallet', async () => {
    try {
      ...
      expect(addVCtoCloud.data.result).toBe(RESULT.SUCCESS)
    } catch (err) {
  ...
  test('Add VC to cloud wallet - Send request with incorrect did_address', async () => {
    try {
      const issRegister = await DID.Register(KEY_TYPE.EcdsaSecp256r1VerificationKey2019)
      console.log('Issuer Register: ', JSON.stringify(issRegister.request.data, null, 2))
      expect(issRegister.request.status).toEqual(201)
      ...
      const addVCtoCloud = await Wallet.AddVCtoCloud(jwtVc, dummyDid, state.didKey1, state.data.did)
      console.log('Add VC to Cloud: ', JSON.stringify(addVCtoCloud.data, null, 2))
      expect(addVCtoCloud.status).toEqual(400)
    } catch (err) {
      console.log(err.response)
  ...
  test('Add VC to cloud wallet - Send request with incorrect JWT format', async () => {
    try {
      const issRegister = await DID.Register(KEY_TYPE.EcdsaSecp256r1VerificationKey2019)
      console.log('Issuer Register: ', JSON.stringify(issRegister.request.data, null, 2))
      expect(issRegister.request.status).toEqual(201)
      ...
      const dummyJwt = faker.name.firstName() + `'sDocument` + '_Type'
      const addVCtoCloud = await Wallet.AddVCtoCloud(dummyJwt, state.data.did, state.didKey1, state.data.did)
      console.log('Add VC to Cloud: ', JSON.stringify(addVCtoCloud.data, null, 2))
      expect(addVCtoCloud.status).toEqual(400)
    } catch (err) {
      console.log(err.response)
      expect(err).toBeTruthy()
      expect(err.response.status).toEqual(400)
      expect(err.response.data.code).toBe(INVALID.JWT.CODE)
      expect(err.response.data.message).toBe(INVALID.JWT.MESSAGE)
    }
  })

  test('Add VC to cloud wallet - Send request without did_address', async () => {
    try {
      const issRegister = await DID.Register(KEY_TYPE.EcdsaSecp256r1VerificationKey2019)
      console.log('Issuer Register: ', JSON.stringify(issRegister.request.data, null, 2))
      expect(issRegister.request.status).toEqual(201)
      ...
      const addVCtoCloud = await Wallet.AddVCtoCloud(jwtVc, '', state.didKey1, state.data.did)
      console.log('Add VC to Cloud: ', JSON.stringify(addVCtoCloud.data, null, 2))
      expect(addVCtoCloud.status).toEqual(400)
    } catch (err) {
      console.log(err.response)
  ...
  test('Add VC to cloud wallet - Send request without JWT', async () => {
    try {
      const didRegister = await DID.Register(KEY_TYPE.EcdsaSecp256r1VerificationKey2019)
      console.log('DID Register: ', JSON.stringify(didRegister.request.data, null, 2))
      expect(didRegister.request.status).toEqual(201)
      ...
      const addVCtoCloud = await Wallet.AddVCtoCloud('', state.data.did, state.didKey1, state.data.did)
      console.log('Add VC to Cloud: ', JSON.stringify(addVCtoCloud.data, null, 2))
      expect(addVCtoCloud.status).toEqual(400)
    } catch (err) {
      console.log(err.response)
      ...
      expect(err.response.data.code).toBe(INVALID.PARAMS.CODE)
      expect(err.response.data.message).toBe(INVALID.PARAMS.MESSAGE)
      expect(err.response.data.fields.jwt.code).toBe(ERR_REQUIRE.JWT.CODE)
      expect(err.response.data.fields.jwt.message).toBe(ERR_REQUIRE.JWT.MESSAGE)
    }
  })

  test('Add VC to cloud wallet by other\'s did & other\'s private_key', async () => {
    try {
      const issRegister = await DID.Register(KEY_TYPE.EcdsaSecp256r1VerificationKey2019)
      console.log('Issuer Register: ', JSON.stringify(issRegister.request.data, null, 2))
      expect(issRegister.request.status).toEqual(201)
      ...
      const addVCtoCloud = await Wallet.AddVCtoCloud(jwtVc, state.data.did, state.issuerKey1, state.data.did)
      console.log('Add VC to Cloud: ', JSON.stringify(addVCtoCloud.data, null, 2))
      expect(addVCtoCloud.status).toEqual(400)
    } catch (err) {
      console.log(err.response)
